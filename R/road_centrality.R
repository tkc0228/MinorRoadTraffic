...
#' @param parallel logical. If TRUE each zone is processed in parallel using
#'   \code{future.apply}. Defaults to FALSE.
#' @export
#'
road_centrality = function(lines, threshold = 5, parallel = FALSE){
...
# Calculate road centrality -----------------------------------------------
  ...
  zone_fun <- function(i){
    ...
    if (nrow(lines_sub) == 0) return(NULL)
  ...
  if (parallel) {
    ...
    graphs <- future.apply::future_lapply(zones$id, zone_fun, future.seed = TRUE)
  } else {
    graphs <- lapply(zones$id, zone_fun)
  }
